...
| APP_COMMON_NAME | app.common_name | Host's preferred IP | The address (IP or hostname) used as the Common Name for Console's generated certificates, and the address AMT devices use to reach the [CIRA](Features/cira.md) listener. Defaults to the host's detected IP if not set. |
...
| APP_DISABLE_CIRA | app.disable_cira | `true` | Enables or disables [CIRA](Features/cira.md) device-initiated remote access (the `:4433` listener). `true` (default) disables CIRA, its REST endpoints, and CIRA-based profiles; set to `false` to enable. See [CIRA](Features/cira.md) for the full behavior. |
...
| HTTP_ALLOWED_ORIGINS | http.allowed_origins | `https://localhost:8181`, `https://127.0.0.1:8181`, `http://localhost:8181`, `http://127.0.0.1:8181` | Origins allowed to make cross-origin requests and to open a redirection (KVM/SOL/IDER) websocket. Add the origin the UI is served from if it is not Console itself. Do not use `*`: it returns `Access-Control-Allow-Origin: *`, which lets any site read API responses, and it disables cookie auth. While `*` is set the redirection relay refuses cross-origin handshakes outright, so KVM/SOL/IDER will fail from a separately hosted UI. |
| HTTP_ALLOWED_HEADERS | http.allowed_headers | `Origin`, `Accept`, `Content-Type`, `Content-Length`, `Authorization`, `If-Match` | Request headers a cross-origin caller may send. Do not use `*`: browsers take it literally once credentials are enabled, and it never covers `Authorization`. |
| HTTP_ALLOW_CREDENTIALS | http.allow_credentials | `true` | Lets a cross-origin browser send the HttpOnly session cookie. Required when the UI is served from a different origin than the API. Forced off whenever `http.allowed_origins` contains `*`, as CORS forbids that combination. |
