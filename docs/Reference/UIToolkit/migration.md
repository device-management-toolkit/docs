...
- The translation namespace changed from `translations` to `translation`.
- Translation keys have been flattened (e.g. `amtFeatures.features.kvm` is now `kvm.connect`).
- Browser language auto-detection has been removed. The default language is English (`en`).
- Supported languages: English (en), Spanish (es), French (fr), German (de), Japanese (ja), Chinese (zh).

## Removed Dependencies

The following dependencies have been removed in v5:

| Removed Package | Notes |
| :-------------- | :---- |
| `@fortawesome/fontawesome-svg-core` | Icons no longer bundled |
| `@fortawesome/free-solid-svg-icons` | Icons no longer bundled |
| `@fortawesome/react-fontawesome` | Icons no longer bundled |
| `i18next-browser-languagedetector` | Language must be set manually |
| `sass` | SCSS no longer supported; use style props |
| `styled-components` | Replaced by style props |
