...
??? note "v4 (Legacy)"
    ...
    In v4, localization used a different structure:

    - Translation files were located in `ui-toolkit-react/src/public/locales/{lang}/translation.json`
    - The i18n config used `i18next-browser-languagedetector` for automatic browser language detection
    - The namespace was `translations` (plural) instead of `translation`
